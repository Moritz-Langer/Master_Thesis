...
\usepackage{rotating}
\usepackage[hyphens]{url} % Using hyphens for URL breaking
\usepackage{xurl}     % This is usually the most effective for long URLs
\usepackage{hyperref} % Keep hyperref for links, but note the order with biblatex
\usepackage[style=apa,
citestyle=apa,
backend=biber,
]{biblatex} % biblatex should be loaded before hyperref if you want hyperref to link citations

% --- Custom commands for unnumbered sections in TOC with roman numerals ---
% This command creates an unnumbered chapter-like entry that appears in the TOC
% It takes two arguments: #1 for the actual title, #2 for the title in the TOC
\newcommand{\addunnumberedchapter}[2]{%
  \chapter*{#1}% % Unnumbered chapter title
  \addcontentsline{toc}{chapter}{#2}% % Add to TOC as a chapter
}
% --- End custom commands ---
...
% --- Front Matter (Roman Numerals and unnumbered sections) ---
\maketitle

% Reset page numbering for front matter to roman
\pagenumbering{roman}
\setcounter{page}{1} % Start roman numbering from page i
...
\listoftables
\clearpage
% Add Abstract and Kurzfassung
% Note: Using \addunnumberedchapter for consistency with chapter-level entries in report class
\addunnumberedchapter{Abstract}{Abstract}
% Your Abstract content goes here
\input{abstract.tex} % Assuming your abstract is in abstract.tex for better organization
\clearpage

\addunnumberedchapter{Kurzfassung}{Kurzfassung}
% Your Kurzfassung content goes here
\input{kurzfassung.tex} % Assuming your kurzfassung is in kurzfassung.tex
\clearpage
% --- Main Content (Arabic Numerals starting from 1 for Introduction) ---
% Set page numbering for main content to arabic and restart from 1
\pagenumbering{arabic}
\setcounter{page}{1}

% Your custom section numbering setup.
% In the 'report' class, sections are usually numbered as chapter.section (e.g., 1.1, 1.2).
% Your original redefinition `\renewcommand{\thesection}{\arabic{section}}` makes them just 1, 2, 3.
% If you want "Introduction" to be "1 Introduction" as a section, and not "Chapter 1 Introduction",
% then this setup is correct. If you want a "Chapter 1" with sections "1.1", "1.2", then you'd use \chapter{Introduction}
% and remove the `\makeatletter...\makeatother` block.

% Current setup (sections as 1, 2, 3...)
\makeatletter
% \@addtoreset{section}{chapter} % This line is redundant if you don't use \chapter commands for your main sections
...
\makeatother
% Ensure subsubsection appears in ToC (already handled by \setcounter{tocdepth}{3})
